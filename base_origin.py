...
saving_file_name = f'homework_best_model_{timestr}.pt'
...
LR = 5e-4
model = DeepLabV3p(in_channels=3, num_classes=29)
# optimizer = optim.SGD(params=model.parameters(), lr=LR, weight_decay=1e-6)
optimizer = optim.Adam(params=model.parameters(), lr=LR, weight_decay=1e-6)
...
scheduler = CosineAnnealingWarmRestarts(optimizer, T_0=9, T_mult=2, eta_min=0, last_epoch=-1)
# scheduler = CyclicLR(optimizer, base_lr=0.0001, max_lr=0.003, step_size_up=5, step_size_down=5, mode='triangular2', gamma=1.0, scale_fn=None, scale_mode='cycle', cycle_momentum=True, base_momentum=0.8, max_momentum=0.95, last_epoch=-1)
